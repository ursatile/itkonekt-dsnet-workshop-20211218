...
namespace Autobarn.Website.Tests {
    public class ApiTests : IClassFixture<TestWebApplicationFactory<Startup>> {
        ...
        [Fact]
        public async void GET_vehicles_via_discovery_endpoint_succeeds() {
            var client = factory.CreateClient();
            var response = await client.GetAsync("/api");
            var json = await response.Content.ReadAsStringAsync();
            var hal = JsonConvert.DeserializeObject<dynamic>(json);
            Assert.NotNull(hal._links.vehicles.href);
            var vehiclesEndpointHref = (string)hal._links.vehicles.href;
            response = await client.GetAsync(vehiclesEndpointHref);
        ...
        public async void GET_vehicles_returns_vehicle_data() {
            ...
            var json = await response.Content.ReadAsStringAsync();
            var hal = JsonConvert.DeserializeObject<dynamic>(json);
            ((JArray)hal.items).Count.ShouldBeGreaterThan(0);
        }
